...
def _max_deviation(y_true, y_pred):
    return tf.keras.backend.max(_deviations_in_batch(y_true, y_pred), axis=-1)


def _min_deviation(y_true, y_pred):
    return tf.keras.backend.min(_deviations_in_batch(y_true, y_pred), axis=-1)


def _mean_deviation(y_true, y_pred):
    return tf.keras.backend.mean(_deviations_in_batch())
...
b = _max_deviation(a, tf.constant(0., shape=(3,4,5)))
